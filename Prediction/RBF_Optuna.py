...
class RBF:
    ...
    def objective(self, trial, pipeline, Features_train, target_train, Features_test, target_test):
        # Hyperparameter setting
        params = {
            'C': trial.suggest_float('C', 1e-2, 1e3, log=True),
            'gamma': trial.suggest_categorical('gamma', ['scale', 'auto'] + list(np.logspace(-4, 1, 10))),
            'epsilon': trial.suggest_float('epsilon', 0.005, 0.5, log=True)
        }

        # Add inner fold for model performance
        ...
        for step, (train_idx, val_idx) in enumerate(kf.split(Features_train)):
            ...
            # Fresh pipeline per fold
            current_model = SVR(kernel='rbf', **params)
    ...
    def RBF_tuning(self, pipeline, Features_train, target_train, Features_test, target_test, 
                    models_dict, Inner_cv, i, eid_test, save_path=None, pipeline_save_path=None,  RBF_n_jobs=1, n_jobs=1):
        """
        ...
        def evaluate_model(model_name, model_tuple):
            model, _ = model_tuple # Unpack the tuple
            print(f'\nEvaluating {model_name} on fold {i+1}:')

            # Create study object
            study = optuna.create_study(study_name=f'{model_name}_fold_{i+1}', 
                                        direction='minimize', 
            ...
            study.optimize(
                lambda trial: self.objective(
                    trial, pipeline, Features_train, target_train, Features_test, target_test
                ),
                n_trials=self.n_trials,
                n_jobs=RBF_n_jobs # Optuna-specific parallelism
            )
        ...
        if save_path:
            ...
            # Appending metrics if file exists
