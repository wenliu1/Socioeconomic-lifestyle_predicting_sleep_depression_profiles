...
from LinearSVR_GSCV import linearSVR
...
The weight from rCCA was arbitrary, so the correlations between the transformed target scores and the original scores were calculated to adjust for the direction, to ensure higher values indicate more severe symptoms.

The direction of all targets should be flipped.
"""

# Load data
df_ML_fold1 = pd.read_csv('/home/w.liu/UKB/Results/without_image/rCCA/Limited_confounds/MIND_work/Social/GP/Unoverlapped/ML_cross_fold_1_conf_age_sex_MINDscore_work_social_unoverlapped_final.csv', low_memory=False)
...
# Calculate row sums and create target
df_ML_fold1['sum_SRD'] = df_ML_fold1[Trans_columns].sum(axis=1)
target_orig = df_ML_fold1['sum_SRD']
target = -target_orig  
eid = df_ML_fold1['eid']
...
def execute_ml_pipeline(Features, target, joined_confounds, models_dict, Outer_cv, fold, parallel_outer=True, parallel_inner=True):
    """
    ...
    def process_fold(fold_info):
        ...
        # Set n_jobs for parallel or sequential analysis
        n_jobs = -1 if parallel_inner else 1

        # Call EN_tuning method
        results = lm_LinearSVR.LinearSVR_tuning(
            pipeline, Features_train, target_train, Features_test, target_test, 
            models_dict, Inner_cv, i, 
            save_path=metrics_path,  # Same path for all folds
    ...
    fold_info = [(i, fold) for i, fold in enumerate(Outer_cv.split(Features))]

    if parallel_outer: # Run outer folds in parallel
    ...
    print(f"Total pipeline execution time: {time.time() - start:.2f} seconds")
...
for fold in folds:
    ...
    # Initialize model
    lm_LinearSVR = linearSVR()
    models_dict = lm_LinearSVR.models_dict

    execute_ml_pipeline(Features, target, joined_confounds, models_dict, Outer_cv, fold, parallel_outer=True, parallel_inner=True)
